...
x = np.arange(3)
xticks = ['Long interval', 'Short interval', 'Continuous']
labels = ['RL', 'priority', 'default', 'none']
markers = ['o', 'D', 'd', 's']
colors = ['r', 'b', 'cyan', 'k']
...
for id, maze in enumerate(mazes):
    for goal_idx, goal in enumerate(goals):
        ...
        axs.set_ylabel('Delivery rate (%)', fontsize = 26)
        axs.tick_params(labelsize=26)
        axs.yaxis.offsetText.set_fontsize(26)
        axs.legend(loc="upper right", prop={'size': 26})
        fig.tight_layout()
        fig.subplots_adjust(
            top=0.977,
            bottom=0.079,
