...
def big_fcn():
    ...
    Part III: 100% ICM
    """
    filename = envs[env_id]
    algo = '100%'
    # Load data from log
    rawdata3 = pandas.read_csv(os.path.join(dir,'logger', 'maze1',filename), header=0, engine='python')
    ...
    filename = envs[env_id+1]

    algo = '80%'
    # Load data from log
    rawdata2 = pandas.read_csv(os.path.join(dir,'logger', 'maze1', filename), header=0, engine='python')
    ...
    filename = envs[env_id+2]

    algo = '60%'
    # Load data from log
    rawdata1 = pandas.read_csv(os.path.join(dir,'logger', 'maze1', filename), header=0, engine='python')
    variables1 = ['n_updates', 'total_secs', 'tcount', 'epcount',\
                  'eplen', 'best_eplen', \
                  'eprew','recent_best_ext_ret', 'best_ext_ret', 'eprew_recent']
    var_dict1 = {}
    for i in np.arange(len(variables1)):
        var_dict1[variables1[i]] = i
    df1 = rawdata1[variables1]
    num_features = len(variables1)
    data_size = len(df1[variables1[0]].values)
    dataset1 = np.empty((data_size, num_features))
    for idx, feature in enumerate(variables1):
        dataset1[:, idx] = np.array(df1[feature].values, dtype=np.float32)
    dataset1[:, var_dict1['total_secs']]/= 3600.0
    dataset1[:, var_dict1['eplen']]*= 4
    dataset1[:, var_dict1['best_eplen']]*= 4
    ...
    if scope == 'eplen':
        ...
        print('Scope %s not found!'%(scope))
        exit()
    mean, std = moving_avg(dataset1[:,var_dict1[y_label]], window=4)
    axs[0].plot(dataset1[:,var_dict1[x_label]],  mean, linewidth=3 ,color='navy',label=algo+' mean')
    axs[0].fill_between(dataset1[:,var_dict1[x_label]],  mean-std, mean+std, color='deepskyblue')
    axs[0].plot(dataset1[:,var_dict1[x_label]],  dataset1[:,var_dict1[y2_label]], '--', linewidth=2 ,color='navy',label=algo+' best')
    ...
    if scope == 'eplen':
        ...
        exit()
    mean, std = moving_avg(dataset1[:,var_dict1[y_label]], window=4)
    axs[1].plot(dataset1[:,var_dict1[x_label]],  mean, linewidth=3 , color='navy',label=algo+' mean')
    axs[1].fill_between(dataset1[:,var_dict1[x_label]],  mean-std, mean+std, color='deepskyblue')
    axs[1].plot(dataset1[:,var_dict1[x_label]],  dataset1[:,var_dict1[y2_label]], '--', linewidth=2 ,color='navy',label=algo+' best')
...
envs = os.listdir(os.path.join(dir, 'logger','maze1'))
envs = [env for env in envs if env[-7:-4]=='ICM']
envs = np.sort(envs)
scopes = ['eplen', 'eprew']
for env_id, env in enumerate(envs):
 ...
 for scope in scopes:
    ...
    include_best = True
    tag=  env[:-11] + '_%s_100-80_eps.png'%(scope)
    figpath = os.path.join(dir, 'fig', tag)
    best_res = True
    big_fcn()
    #-----------------------------------------------------------